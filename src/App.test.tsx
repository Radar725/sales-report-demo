...
describe('App', () => {
  ...
  it('uses filtered records in the breakdown drawer', async () => {
    ...
    expect(within(drawer).getByRole('columnheader', { name: '新客业绩占比' })).toBeInTheDocument();
    expect(within(drawer).getByRole('columnheader', { name: '新客成交单量占比' })).toBeInTheDocument();
    expect(within(drawer).getByRole('columnheader', { name: '新客成交客户占比' })).toBeInTheDocument();
  ...
  it('uses the same dynamic metric names in the breakdown drawer', async () => {
    ...
    for (const column of [
      ...
      '老客复购客单价', '老客复购业绩占比', '老客复购成交单量占比', '老客复购成交客户占比',
      '老客复购业绩贡献', '老客复购成交单量贡献', '老客复购成交客户贡献',
    ]) {
      expect(within(drawer).getByRole('columnheader', { name: column })).toBeInTheDocument();
    }
  });
